...
import { processEngagementByType, processReachData, processEngagementRate } from '../lib/utils'
...
const MetricCard = ({ title, value, icon, className }: { title: string; value: string; icon: React.ReactNode; className: string }) => {
  return (
...
export default function SocialMediaDashboard() {
  ...
  const filteredData = selectedPostType === 'All' 
    ? mockData 
  ...
  const generateInsights = () => {
    ...
    return `
      Based on the current data for ${selectedPostType === 'All' ? 'all post types' : selectedPostType}:
      1. The average engagement rate is ${(engagementRateData.reduce((sum, item) => sum + item.value, 0) / engagementRateData.length).toFixed(2)}%.
      2. ${engagementByType[0]?.name || 'The selected post type'} has the highest average likes at ${engagementByType[0]?.likes || 0}.
      3. The reach trend shows ${reachData[reachData.length - 1].value > reachData[0].value ? 'an increasing' : 'a decreasing'} pattern.
    `
  }
  ...
  const handleChatSubmit = (e: React.FormEvent) => {
    ...
    setChatResponse(`Here's some information about ${chatInput}: [AI-generated response would go here]`)
    setChatInput('')
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          <h1 className="text-4xl font-bold text-gray-800 dark:text-white mb-4 md:mb-0">Social Media Analytics</h1>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <PostTypeDropdown value={selectedPostType} onChange={setSelectedPostType} />
            <Button onClick={downloadData} className="w-full sm:w-auto">
        ...
        <Tabs defaultValue="dashboard" className="w-full">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                >
                  <MetricCard
                    ...
                    value={filteredData.reduce((sum, post) => sum + post.likes + post.comments + post.shares, 0).toLocaleString()}
                ...
                >
                  <MetricCard
                    title="Average Reach"
                    value={Math.floor(filteredData.reduce((sum, post) => sum + post.reach, 0) / filteredData.length).toLocaleString()}
                ...
                >
                  <MetricCard
                    ...
                    value={`${(filteredData.reduce((sum, post) => sum + post.engagement_rate, 0) / filteredData.length).toFixed(2)}%`}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                >
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-xl font-semibold">Engagement by Post Type</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                >
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-xl font-semibold">Reach Trend</CardTitle>
                    </CardHeader>
                    <CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-xl font-semibold">Engagement Rate by Type</CardTitle>
                    </CardHeader>
                    <CardContent>
          ...
          <TabsContent value="insights">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl font-semibold">Data Insights</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="whitespace-pre-line text-gray-700 dark:text-gray-300">{generateInsights()}</p>
              </CardContent>
            </Card>
          </TabsContent>
          <TabsContent value="chatbot">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl font-semibold">Data Chatbot</CardTitle>
              </CardHeader>
              <CardContent>
